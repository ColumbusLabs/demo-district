...
export interface WorldOptions {
  ...
  /** Supplied by the development HUD only; absent in production. */
...
/**
 * Owns one renderer, scene, camera, frame loop, and all lifetime subscriptions.
 * Creation does not start a loop. Destroy is terminal/idempotent and retires the
 * canvas's context; use a fresh canvas for re-entry (the app mount does this).
 */
export function createWorld(canvas: HTMLCanvasElement, options: WorldOptions = {}): World {
  ...
  const cleanup = (): void => {
    pauseLoop();
    // Remove app listeners before intentionally losing this retired context.
  ...
  try {
    ...
    const render = (time: number, animate: boolean): void => {
      if (destroyed || failed || contextLost || !drawable || doc.hidden || !requested) return;
      try {
        const tick = animate ? clock.tick(time) : { delta: 0, elapsed: elapsedSeconds };
        lastDeltaSeconds = tick.delta;
        elapsedSeconds = tick.elapsed;
        if (animate) fixture.update(tick.delta);
    ...
    const frame = (time: number): void => { render(time, true); };
    ...
    const resize = (): void => {
      ...
      if (next && changed) {
        // One allocation; CSS size remains governed by the layout, not Three.js.
    ...
    // DPR can change between monitors/at browser zoom without a layout resize.
    ...
    destroyed = true;
    try { cleanup(); } catch { /* Preserve the initialization error after best-effort cleanup. */ }
    throw error;
  }
}
